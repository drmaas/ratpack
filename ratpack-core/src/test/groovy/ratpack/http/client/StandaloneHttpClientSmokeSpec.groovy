...
class StandaloneHttpClientSmokeSpec extends BaseHttpClientSpec {

  PollingConditions polling = new PollingConditions()

  def <T> T resolve(Supplier<Promise<T>> promise) {
    T result = null
  ...
  def "can make simple get request"() {
    ...
    def standaloneClient = HttpClient.of(ExecController.create(), { it.poolSize(pooled ? 8 : 0) })

    when:
    text = resolve { standaloneClient.get(otherAppUrl("foo")) }.body.text
